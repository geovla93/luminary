...
const EthereumLegacyInput = ({onChange}: IProps) => {
  const [customIndex, setCustomIndex] = useState<string>('n');

  const handlePathChange = (value: string) => {
    if (value === '') {
      setCustomIndex('n');
      onChange("m/44'/60'/0'/n");
      return;
    } else if (value.includes('n') && value.length > 1) {
      setCustomIndex(value.replace('n', ''));
      onChange(`m/44'/60'/0'/${value.replace('n', '')}`);
      return;
    }
    setCustomIndex(value);
    onChange(`m/44'/60'/0'/${value}`);
  };

  return (
    <View style={styles.root}>
      <Typography variant="bodySmall">m/44'/60'/0'/</Typography>
...
export default EthereumLegacyInput;
